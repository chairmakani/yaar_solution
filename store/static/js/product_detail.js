document.addEventListener('DOMContentLoaded', function () {
    ...
    class ProductManager {
        constructor() {
            this.variantSelectors = document.querySelectorAll('.variant-choice input');
            this.addToCartBtn = document.querySelector('.add-to-cart-btn');
            this.quantityInput = document.querySelector('.quantity-input');
            this.priceDisplay = document.querySelector('.current-price');
            this.stockDisplay = document.querySelector('.stock-status');
        ...
        initializeListeners() {
            this.variantSelectors.forEach(input => {
                input.addEventListener('change', (e) => this.handleVariantChange(e));
            });
        ...
        handleVariantChange(e) {
            ...
            const variantData = {
                ...
                stock: parseInt(variant.dataset.stock)
            };

            this.updateUI(variantData);
        }

        updateUI(variantData) {
            // Update price
            this.priceDisplay.textContent = `₹${variantData.price}`;
            
            // Update stock status and button state
            const inStock = variantData.stock > 0;
            this.addToCartBtn.disabled = !inStock;
            this.addToCartBtn.innerHTML = inStock ? 
                '<i class="fas fa-cart-plus"></i> Add to Cart' : 
                '<i class="fas fa-times"></i> Out of Stock';
            
            // Update quantity limits
            if (this.quantityInput) {
                this.quantityInput.max = variantData.stock;
            ...
            if (this.stockDisplay) {
                this.stockDisplay.className = `stock-status ${inStock ? 'in_stock' : 'out_of_stock'}`;
                this.stockDisplay.textContent = inStock ? 
                    `${variantData.stock} available` : 
                    'Out of Stock';
            }
        }
        ...
        async handleAddToCart(e) {
            ...
            if (requiresVariant && !selectedVariant) {
                NotificationManager.show('Please select a variant first', 'warning');
                return;
            }
            ...
            await this.addToCart(button, productData);
        }
    }
    ...
    class VariantManager {
        constructor() {
            this.variantInputs = document.querySelectorAll('.variant-choice input');
            this.addToCartBtn = document.querySelector('.add-to-cart-btn');
            this.priceDisplay = document.querySelector('#display-price');
            this.init();
        }
        ...
        init() {
            ...
            if (this.addToCartBtn?.dataset.requiresVariant === 'true') {
        ...
        handleVariantChange(e) {
            ...
            this.updateUI(variantData);
            this.updateButtonState(true, variantData.stock > 0);
        }
        ...
        updateUI(variantData) {
            ...
            // Update stock status
            const stockLabel = document.querySelector('.stock-status');
            if (stockLabel) {
                this.updateStockStatus(stockLabel, variantData.stock);
            }
        }

        updateButtonState(variantSelected, inStock = true) {
            if (!this.addToCartBtn) return;

            this.addToCartBtn.classList.remove('select-options', 'add-to-cart');
            
            if (!variantSelected) {
                this.addToCartBtn.innerHTML = '<i class="fas fa-list"></i> Select Options';
                this.addToCartBtn.classList.add('select-options');
        ...
        updateStockStatus(element, stock) {
            ...
            element.className = `stock-status ${className}`;
        }
    }
